...
func (r *RawSQL[T]) setFields(res *sql.Rows) (*T, error) {
	...
	receiptFields := make([]any, 0, len(r.models.Fields))                    // 保存Scan需要数据
	receiptInterfaceFields := make([]reflect.Value, 0, len(r.models.Fields)) // 用于保存每个字段的 Value类型
	for _, str := range orderColumnsStr {
		fd, ok := r.models.ColumnsMap[str]
		if !ok {
...
func (r *RawSQL[T]) QueryRawWithContext(ctx context.Context) (*T, error) {
	...
	return r.setFields(res)
}

func (r *RawSQL[T]) Build() (*SQLInfo, error) {
	var err error
	r.models, err = r.db.manager.Get(new(T))
	if r.sql == "" {
